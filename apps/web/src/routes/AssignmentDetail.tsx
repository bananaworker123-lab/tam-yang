...
export function AssignmentDetailPage() {
  ...
  if (!p) return <StatusBanner kind="error">Assignment not found. It may have been removed.</StatusBanner>;
  ...
  const dueChipLabel = due === 'overdue' ? `Overdue · ${fmtDate(p.dueDate)}` : due === 'due_today' ? `Due today · ${fmtDate(p.dueDate)}` : due === 'near' ? `Due soon · ${fmtDate(p.dueDate)}` : p.status === ProgressStatus.Submitted ? 'Submitted ✓' : `Due ${fmtDate(p.dueDate)}`;
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-2.5 mb-5">
        <InfoBox label="Assigned" value={fmtDate(p.assignedDate)} />
        <InfoBox label="Due" value={fmtDate(p.dueDate)} />
        <InfoBox label="Class" value={p.className} />
        <InfoBox label="Term" value={p.term} />
      </div>

      <div className="bg-white border border-line rounded-[18px] p-4">
        <div className="font-bold text-[14.5px] text-ink mb-3">Homework status</div>
        {isTeacher ? (
          <div className="text-sm text-muted bg-bg rounded-xl px-3 py-3">Read-only — teachers can't change status.</div>
        ) : (
