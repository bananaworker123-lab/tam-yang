...
export class FamilyService {
  ...
  async createInvite(familyId: string, role: 'parent' | 'child') {
    const token = randomBytes(32).toString('hex');
    const invite = await this.prisma.invite.create({
      data: { familyId, email: '', role, token },
    });
    this.logger.log(`QR invite created for family ${familyId} role=${role} token=${token.slice(0, 8)}…`);
    return { inviteId: invite.id, token };
  }

  async acceptInvite(token: string, _googleEmail: string, userId: string) {
